...
module atomic_data
  ...
  data f_O7 / 0.696 /
  real(kind=doubleR) :: Lambda_O8(1), f_O8(1)
  data Lambda_O8 / 18.969 /      
  data f_O8 / 0.416 /
  !
  ! Ne
...
module runtime
  ...
  !
  character(len=300) :: parameter_file = 'specwizard.par'  ! The parameter filename, read from command line
  ...
  character(len=300) :: SpectrumFile = invalid        ! output file name, overrides automatic generation
  ...
  real(kind=singleI), allocatable :: axis_array(:)
  real(kind=doubleR) :: phi_universal = 0. !BDO
  real(kind=doubleR) :: theta_universal = 0. !BDO
  ...
  logical  :: use_gaussian_kernel = .true.           ! use a normalized, truncated gaussian kernel rather than M4 ! BDO change to true 9/7/17
  logical  :: integrate_kernel = .true.              ! integrate kernel exactly over whole pixel, rather than using r-pixel value ! BDO changed to true 9/7/17
...
module spectra
  ...
  character(len=300), allocatable     :: simfile(:)
  ...
  character(len=300), allocatable    :: losfile_used(:)
  ...
  real(kind=doubleR), allocatable  :: n_ion(:,:), temp_ion(:,:), met_ion(:,:), veloc_ion(:,:), od_ion(:,:)
  !
  ! redshift-space quantities weigthed by number of ions
  real(kind=doubleR), allocatable  :: rho_z_ion(:,:), temp_z_ion(:,:), met_z_ion(:,:), tau_ion(:,:), &
  ...
  real(kind=doubleR), allocatable  :: tau(:), cdens(:), &
    vhubble(:), nionw(:),rhow(:),tempw(:), metw(:), velocw(:), work(:), work2(:), voc(:), vocsim(:)
  !
end module spectra
...
module noisedata
  ...
  character(len=300) :: noisefile = invalid
  !
...
module modified_metallicity
  ...
  character(len=300) :: particle_file_name = invalid
...
module particledata
...
#ifdef CHEMARRAY
  real(kind=singleR), allocatable:: HI(:), CII(:), CIII(:), CIV(:), OVI(:), OVII(:), OVIII(:), SiII(:), SiIII(:), SiIV(:), MgII(:), NV(:)
#endif
  ...
  integer(kind=singleI) :: H_index=-1, He_index=-1, C_index=-1, N_index=-1, &
    ...
    Fe_index=-1
  logical :: requireH=.false., requireHe=.false., requireC=.false., requireN=.false., &
