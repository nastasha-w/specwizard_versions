...
contains
 ...
 subroutine set_sampling_rate(snap, rate)
  ...
  function get_particle_locations(snap, itype, &
    ...
    citype = itype
    cnmax = min(size(file_index), size(file_offset))
  ...
  function read_dataset_int4_1d(snap, itype, name, buf, iostat) result(np)
    ...
    integer,                    optional :: iostat
    integer(kind=C_LONG_LONG)            :: cnp
    integer(kind=C_INT)                  :: citype, csize
    ...
    citype = itype
    csize  = size(buf)
    call readdatasetf(cnp, snap, citype, int(0,C_INT), buf, csize, trim(name)//achar(0))
    np = cnp
    if(present(iostat))then
       if(np.lt.0)then
          iostat = -1
  ...
  function read_dataset_int8_1d(snap, itype, name, buf, iostat) result(np)
    ...
    citype = itype
    csize  = size(buf)
    call readdatasetf(cnp, snap, citype, int(1,C_INT), buf, csize, trim(name)//achar(0))
    np = cnp
    if(present(iostat))then
       if(np.lt.0)then
          iostat = -1
  ...
  function read_dataset_real4_1d(snap, itype, name, buf, iostat) result(np)
    ...
    citype = itype
    csize  = size(buf)
    call readdatasetf(cnp, snap, citype, int(2,C_INT), buf, csize, trim(name)//achar(0))
    np = cnp
    if(present(iostat))then
       if(np.lt.0)then
          iostat = -1
  ...
  function read_dataset_real8_1d(snap, itype, name, buf, iostat) result(np)
    ...
    citype = itype
    csize  = size(buf)
    call readdatasetf(cnp, snap, citype, int(3,C_INT), buf, csize, trim(name)//achar(0))
    np = cnp
    if(present(iostat))then
       if(np.lt.0)then
          iostat = -1
